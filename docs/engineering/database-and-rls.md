...
- **Spot delete + feed events**: deleting a Spot cascades to `spot_likes` / `spot_bookmarks`, whose AFTER DELETE triggers call `_record_feed_event_for_user_v1`. That helper (and `record_feed_event_v1`) now no-ops when the Spot row is missing instead of inserting into `user_feed_events` and violating `user_feed_events_spot_id_fkey` mid-cascade (`20260809174457_fix_feed_event_skip_missing_spot_on_cascade_delete.sql`).
