...
- **Saved Spots and collections**: `spot_bookmarks` is the canonical saved relationship and is unique on `(user_id, spot_id)`; `bookmark_collection_spots` is optional grouping metadata and is unique on `(collection_id, spot_id)`. `remove_saved_spot_v1(uuid)` atomically removes the caller's collection memberships before the canonical save. See `20260808220600_canonical_spot_saves_v1.sql`.
