-- Vibe-to-photo mapping (PHOTO_SYNCED) + display mode on spots.
-- Default remains rotating; existing posts need no backfill.
...
alter table public.spots
  add column if not exists vibe_display_mode text not null default 'rotating';

alter table public.spots
  drop constraint if exists spots_vibe_display_mode_check;

alter table public.spots
  add constraint spots_vibe_display_mode_check
  check (vibe_display_mode in ('rotating', 'photo_synced'));

comment on column public.spots.vibe_display_mode is
  'rotating = post-level auto-rotate vibes; photo_synced = each image has one mapped vibe.';

alter table public.spot_images
  add column if not exists vibe_tag_id uuid references public.vibe_tags (id);

comment on column public.spot_images.vibe_tag_id is
  'When the parent spot is photo_synced, the vibe mapped to this image. Null for rotating.';

create unique index if not exists spot_images_spot_vibe_tag_uidx
  on public.spot_images (spot_id, vibe_tag_id)
  where vibe_tag_id is not null;

-- ---------------------------------------------------------------------------
-- publish_spot_with_approved_media_assets_v1
-- When photo_synced, p_vibe_tag_ids must be ordered 1:1 with p_media_asset_ids.
-- ---------------------------------------------------------------------------

drop function if exists public.publish_spot_with_approved_media_assets_v1(
  uuid[], double precision, double precision, text, uuid[]
);

drop function if exists public.publish_spot_with_approved_media_assets_v1(
  uuid[], double precision, double precision, text, uuid[], text
);
...
create or replace function public.publish_spot_with_approved_media_assets_v1(
  ...
  p_vibe_display_mode text default 'rotating'
)
returns uuid
language plpgsql
security definer
...
declare
  ...
  v_i int;
  v_vid uuid;
  v_mode text := lower(trim(coalesce(p_vibe_display_mode, 'rotating')));
  v_image_vibe uuid;
begin
  ...
  if v_mode not in ('rotating', 'photo_synced') then
    raise exception 'invalid vibe_display_mode';
  end if;
  ...
  if v_mode = 'photo_synced' then
    if not v_is_pro then
      raise exception 'Matching vibes to photos is a Pro feature.';
    end if;
    if n < 2 or v_nv < 2 then
      raise exception 'photo_synced requires at least 2 photos and 2 vibes';
    end if;
    if n <> v_nv then
      raise exception 'photo_synced requires equal photo and vibe counts';
    end if;
  end if;
  ...
  loop
    ...
    v_image_vibe := case
      when v_mode = 'photo_synced' then p_vibe_tag_ids[v_idx + 1]
      else null
    end;
...
revoke all on function public.publish_spot_with_approved_media_assets_v1(
  uuid[], double precision, double precision, text, uuid[], text
) from public;

grant execute on function public.publish_spot_with_approved_media_assets_v1(
  uuid[], double precision, double precision, text, uuid[], text
) to authenticated;

comment on function public.publish_spot_with_approved_media_assets_v1(
  uuid[], double precision, double precision, text, uuid[], text
) is
  'Publishes a Spot from approved media assets. Optional p_vibe_display_mode=photo_synced maps vibes 1:1 to images by order.';

-- ---------------------------------------------------------------------------
-- update_spot_editor_v1 (create or replace with vibe photo sync)
-- ---------------------------------------------------------------------------

drop function if exists public.update_spot_editor_v1(
  uuid, uuid[], double precision, double precision, text, jsonb
);

drop function if exists public.update_spot_editor_v1(
  uuid, uuid[], double precision, double precision, text, jsonb, text
);
...
begin
  ...
  select s.user_id, s.vibe_display_mode
    into v_owner, v_current_mode
  from public.spots s
  ...
  -- Force rotating when counts no longer support a bijection.
  if v_mode = 'photo_synced'
     and (v_final_image_count < 2
          or v_final_vibe_count < 2
          or v_final_image_count <> v_final_vibe_count) then
    v_mode := 'rotating';
  end if;

  -- Enabling photo_synced requires Pro; keeping an already-synced post is allowed.
  if v_mode = 'photo_synced'
     and not v_is_pro
     and coalesce(v_current_mode, 'rotating') <> 'photo_synced' then
    raise exception 'Matching vibes to photos is a Pro feature.';
  end if;
  ...
  -- Clear before reassignment so vibe swaps cannot trip the unique
  -- (spot_id, vibe_tag_id) partial index mid-update.
  if v_mode = 'photo_synced' then
    update public.spot_images
    set vibe_tag_id = null
    where spot_id = p_spot_id;
  end if;
  ...
  for v_item in select value from jsonb_array_elements(p_media_items) loop
    ...
    v_media_asset_id := nullif(v_item->>'media_asset_id', '')::uuid;
    v_image_vibe := case
      when v_mode = 'photo_synced' then p_vibe_tag_ids[v_index + 1]
      else null
    end;
  ...
  -- Clear any leftover image vibes when rotating.
...
comment on function public.update_spot_editor_v1(
  ...
  'Atomically updates an owned Spot after replacement images pass moderation. Supports optional photo-synced vibe mapping.';
